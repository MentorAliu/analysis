...
internal sealed class JsonHttp : IDisposable
{
    private readonly HttpClient client;

    public JsonHttp(Uri origin, HttpMessageHandler? handler = null, Func<CancellationToken, Task>? beforeAttempt = null)
    {
    ...
    private readonly Func<CancellationToken, Task>? beforeAttempt;

    public async Task<byte[]> GetAsync(string path, CancellationToken cancellationToken)
    {
        if (path.Length > 2048 || !path.StartsWith('/') || path.StartsWith("//") || path.Contains('\\') || path.Contains('#'))
            throw new ArgumentException("Only relative API paths are permitted.");
        for (var attempt = 0; attempt < 3; attempt++)
        {
            try
            {
                if (beforeAttempt is not null) await beforeAttempt(cancellationToken);
                using var request = new HttpRequestMessage(HttpMethod.Get, path);
                request.Headers.Accept.ParseAdd("application/json");
                request.Headers.UserAgent.ParseAdd("AnalysisPrivateResearch/0.1");
                ...
                if (!response.IsSuccessStatusCode)
                    ...
                    {
                        HttpStatusCode.Unauthorized or HttpStatusCode.Forbidden or (HttpStatusCode)451 => "access-denied",
